...
export default function OrderSummary() {
  ...
  const deleteCardById = (id) => {
    let newCard = products.filter((product) =>product.id !== id);
    setProducts(newCard);
  };
  const sumCard = () => {
      ...
      <div className='mt-4 rounded-lg border border-gray-200 bg-white shadow-sm'>
                ...
                <div className='flex'>
                  ...
                  <div className='ml-4 flow-root flex-shrink-0'>
                    <button
                      ...
                      key={product.id}
                      onClick={()=>deleteCardById(product.id)}
                    >
        ...
        <dl className='space-y-6 border-t border-gray-200 px-4 py-6 sm:px-6'>
          <div className='flex items-center justify-between'>
            ...
            <dd className='text-sm font-medium text-gray-900'>${sumCard().toFixed(2)}</dd>
          </div>
          <div className='flex items-center justify-between'>
            <dt className='text-sm'>Kargo</dt>
            <dd className='text-sm font-medium text-gray-900'>$ {cargo.toFixed(2)}</dd>
          </div>
          <div className='flex items-center justify-between border-t border-gray-200 pt-6'>
            <dt className='text-base font-medium'>Toplam</dt>
            <dd className='text-base font-medium text-gray-900'>${(sumCard()+cargo).toFixed(2)}</dd>
          </div>
        </dl>
